...
class ImageCropAndResize(DataProcessingOperator):
    ...
    def dimensions_for_pixel_budget(self, image: Image.Image) -> tuple[int, int]:
        """Return (height, width) with W*H <= max_pixels, aspect preserved, dims aligned to division factors."""
        width, height = image.size
        target_area = self.max_pixels if self.max_pixels is not None else width * height
        w0, h0 = width, height
        if w0 > 0 and h0 > 0 and target_area > 0:
