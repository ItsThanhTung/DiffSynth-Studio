...
class WanVideoPipeline(BasePipeline):
    ...
    def from_pretrained(
        ...
        if tokenizer_config is not None:
            ...
            if not tok_path:
                raise RuntimeError(
                    "Tokenizer path could not be resolved (empty glob under ./models). "
                    "Set tokenizer_config to a local directory (e.g. <Wan>/google/umt5-xxl) or use precomputed T5 embeddings."
                )
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("height", "width", "num_frames", "seed", "rand_device", "vace_reference_image", "input_latents"),
            output_params=("noise",)
        )

    def process(self, pipe: WanVideoPipeline, height, width, num_frames, seed, rand_device, vace_reference_image, input_latents=None):
        if input_latents is not None and pipe.scheduler.training and vace_reference_image is None:
            shape = tuple(input_latents.shape)
            noise = pipe.generate_noise(shape, seed=seed, rand_device=rand_device)
            return {"noise": noise}
        if pipe.vae is None:
            raise RuntimeError(
                "VAE is not loaded. Either include Wan2.1_VAE in model configs, or use precomputed video latents "
                "(input_latents from wan_latent_cache) so noise shape can be inferred without VAE."
            )
    ...
    def process(self, pipe: WanVideoPipeline, input_latents, input_video, input_image, noise, tiled, tile_size, tile_stride, vace_reference_image, framewise_decoding, animate_pose_video, animate_face_video):
        if input_video is None:
            if input_latents is not None and pipe.scheduler.training:
                ...
                return {"latents": noise, "input_latents": il}
...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            ...
            input_params=("precomputed_context",),
    ...
    def process(self, pipe: WanVideoPipeline, prompt, positive, precomputed_context=None) -> dict:
        if precomputed_context is not None:
            ctx = precomputed_context.to(device=pipe.device, dtype=pipe.torch_dtype)
            return {"context": ctx}
        if pipe.tokenizer is None:
            raise RuntimeError(
                "Tokenizer is not loaded. Use precomputed T5 embeddings (column t5_context) or load the UMT5 tokenizer."
            )
        if pipe.text_encoder is None:
            raise RuntimeError(
                "Text encoder is not loaded but no precomputed T5 embeddings were provided. "
                "Pass `precomputed_context` (training: column `t5_context` / tensor) or include T5 weights in model configs."
            )
        pipe.load_models_to_device(self.onload_model_names)
...
class WanVideoUnit_ImageEmbedderCLIP(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("clip_feature", "input_image", "end_image", "height", "width"),
            output_params=("clip_feature",),
            onload_model_names=("image_encoder",)
        )

    def process(self, pipe: WanVideoPipeline, clip_feature, input_image, end_image, height, width):
        if clip_feature is not None:
            return {"clip_feature": clip_feature.to(dtype=pipe.torch_dtype, device=pipe.device)}
        if input_image is None or pipe.dit is None or not pipe.dit.require_clip_embedding:
            return {}
        if pipe.image_encoder is None:
            raise RuntimeError(
                "CLIP / image encoder is not loaded but no precomputed `clip_feature` was provided. "
                "Include CLIP weights in model configs or precompute embeddings (see precompute_t5_clip_embeddings.py)."
            )
        pipe.load_models_to_device(self.onload_model_names)
...
class WanVideoUnit_AnimatePoseLatents(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("pose_latents", "animate_pose_video", "tiled", "tile_size", "tile_stride"),
            output_params=("pose_latents",),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoPipeline, pose_latents, animate_pose_video, tiled, tile_size, tile_stride):
        if pose_latents is not None:
            return {"pose_latents": pose_latents.to(dtype=pipe.torch_dtype, device=pipe.device)}
